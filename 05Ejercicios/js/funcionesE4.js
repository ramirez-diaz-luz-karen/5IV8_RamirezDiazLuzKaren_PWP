function validarC(e){
    ...
    if (partes[0].length > 2) return false; // Bloquea más de 2 dígitos antes del punto
...
function calcuarCalificacionFinal(){
    var calParcial = document.getElementById("calificacionP").value;
    var calExamen = document.getElementById("examenF").value;
    var calTrabajo = document.getElementById("trabajoF").value;

    var parseo1 = parseFloat(calParcial);
    var parseo2 = parseFloat(calExamen);
    var parseo3 = parseFloat(calTrabajo);

    if (isNaN(parseo1) || isNaN(parseo2) || isNaN(parseo3)) {
        alert("Por favor ingresa valores válidos.");
        return;
    }

    if (parseo1 < 0 || parseo1 > 10 || parseo2 < 0 || parseo2 > 10 || parseo3 < 0 || parseo3 > 10) {
